...
$updateSuccess = false; // Flag to check if update was successful

// Check if form is submitted
if(isset($_POST['submit'])) {
    $ids = $_POST['ids']; // Assuming you have an input for the record ID to update
    ...
    // Randomly select jeepicon (assuming this works as intended)
    ...
    $sql = "UPDATE `locate` SET `drivername`='$name', `cnumber`='$number', `platenumber`='$pnumber', 
                `route`='$route', `jeepicon`='$jeepicon', `address`='$address', 
...
// Fetch existing record for editing (if ID is provided)
if (isset($_GET['ids'])) {
    $id = $_GET['id'];
    $ids = $_GET['ids'];
    ...
    if ($result->num_rows > 0) {
        ...
        $name = $row['drivername'];
        $number = $row['cnumber'];
        // Separate address components if stored as concatenated string
        ...
        // $jeepicon = $row['jeepicon']; // Assuming jeepicon is fetched or set elsewhere
    } else {
    ...
    <script type="text/javascript">
        ...
        <?php if ($updateSuccess): ?>
            swal({
                ...
                window.location.href = "dashboard.php?id=<?php echo $id ?>"; // Redirect to dashboard or appropriate page
            });
        <?php endif; ?>
    </script>
</body>

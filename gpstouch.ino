...
// Add these constants at the top
const char* AGPS_URL = "https://online-live1.services.u-blox.com/GetOnlineData.ashx";
const char* AGPS_TOKEN = "e4ZpbQrkS1mNxjscBuinHA";  // Get this from u-blox
...
void setup() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        updateAGPS();  // Get A-GPS data on startup
    }
}
...
void loop() {
    ...
    static unsigned long lastHttpMillis = 0;
    const unsigned long SEND_INTERVAL = 1000;  // Increased to 1 second
    const unsigned long HTTP_MIN_INTERVAL = 500;  // Minimum time between HTTP updates
    static unsigned long lastAgpsUpdate = 0;
    const unsigned long AGPS_UPDATE_INTERVAL = 4 * 60 * 60 * 1000;  // 4 hours
    ...
    if (packetSize) {
        ...
        if (receiver == DEVICE_1_ADDRESS && sender == DEVICE_2_ADDRESS) {
            ...
            if (message == "BUTTON2_PRESSED") {
                Slot++;
                triggerBuzzer();
                // Only update HTTP if enough time has passed
                if (currentMillis - lastHttpMillis >= HTTP_MIN_INTERVAL) {
                    forceHttpUpdate();
                    lastHttpMillis = currentMillis;
                }
            } else if (message == "BUTTON1_PRESSED") {
                if (Slot > 0) {
                    Slot--;
                    triggerBuzzer();
                    if (currentMillis - lastHttpMillis >= HTTP_MIN_INTERVAL) {
    ...
    // Handle sending with proper interval
    ...
    // Update A-GPS data every 4 hours
    if (millis() - lastAgpsUpdate > AGPS_UPDATE_INTERVAL) {
        updateAGPS();
        lastAgpsUpdate = millis();
    }
}
...
void triggerBuzzer() {
    for (int i = 0; i < 2; i++) { // Reduced from 5 to 2 beeps
        ...
        delay(100);  // Reduced from 100ms to 50ms
...
void updateAGPS() {
    if (WiFi.status() == WL_CONNECTED) {
        HTTPClient http;
        String url = String(AGPS_URL) + 
                    "?token=" + AGPS_TOKEN + 
                    "&gnss=gps&format=aid&datatype=eph,alm,aux";
        ...
        if (httpCode == HTTP_CODE_OK) {
            ...
            // Send A-GPS data to NEO-8M
            while (http.connected() && (len > 0 || len == -1)) {
                size_t size = stream->available();
                if (size) {
                    int c = stream->readBytes(buff, ((size > sizeof(buff)) ? sizeof(buff) : size));
                    Serial2.write(buff, c);  // Send to GPS module
                    if (len > 0) {
                        len -= c;
                    }
                }
            ...
            Serial.println("A-GPS data updated successfully");
        }
        http.end();
    }
}
